...
def load_and_clean(path: str) -> pd.DataFrame:
    """Load CSV, drop rows with missing review_text, strip HTML tags."""
    try:
        df = pd.read_csv(path)
...
def clean_text(text: str) -> str:
    """Strip HTML tags, remove non-alpha characters, lowercase."""
...
def tokenize(text: str) -> list:
    """Tokenize cleaned text; drop stop words and single-char tokens."""
...
def compute_tf(tokens: list) -> dict:
    ...
    Returns a dict of {term: tf_value}.
    """
    if not tokens:
        return {}
...
def compute_idf(corpus_tokens: list) -> dict:
    """
    Smoothed IDF (sklearn-compatible):
...
def tfidf_from_scratch(corpus: list) -> tuple:
    """
    Build a sparse, L2-normalised TF-IDF matrix from scratch.

    Parameters:
        corpus  : list of raw review strings

    Returns:
        tfidf_csr  : scipy csr_matrix  (n_docs × n_vocab), L2-normalised
        vocab      : sorted list of vocabulary terms
        idf_dict   : dict {term: idf_value}
    """
...
def rank_top_reviews(
    ...
    """
    Vectorise a query and rank all reviews by cosine similarity.
    Query is represented as a raw-count L2-normalised vector.
    """
    term2idx   = {t: i for i, t in enumerate(vocab)}
    ...
    scores  = tfidf_matrix.dot(query_vec)          # returns np.ndarray
...
def compare_with_sklearn(corpus: list, scratch_matrix: csr_matrix) -> float:
    """
    Fit sklearn TfidfVectorizer on the same cleaned corpus.
    Return average per-row L2 norm of (scratch − sklearn) on first 500 docs.

    Note: exact match is not expected — sklearn uses slightly different
    tokenisation (regex r'(?u)\\b\\w\\w+\\b') and the same smoothed IDF formula.
    The ranking behaviour is what matters, not the exact float values.
    """
...
def top_word_in_category(
    ...
    """Return (word, avg_score) for the word with highest mean TF-IDF
    across all documents belonging to `category`."""
